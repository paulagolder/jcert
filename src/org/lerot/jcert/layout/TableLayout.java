		...
		for (int i = 0; i < ncomponents; i++) {
			...
			if (comp.isVisible()) {
				...
				if (colspan > 1) {
					...
					width = (int) colsettings.size;
				comp.setBounds((int) (x * cscale), (int) (y * rscale),
						(int) (width * cscale), (int) (height * rscale));
				//comp.setLocation((int) (x * cscale), (int) (y * rscale));
				int pborderwidth = ((TablePanel)parent).getCellborderwidth();
				if(pborderwidth>0)
				{
				((jcertPanel) comp).setBorder(utils.setborder(((TablePanel)parent).getCellbordercolor(), pborderwidth));
				}
