...

{
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if (command.equals("saveas")) {
			...
			if (!savefile.startsWith("/"))
				savefile = JCert_gui.certificatepath + savefile;
		...
		if (command.equals("load")) {
			...
			fc.setCurrentDirectory(new File(JCert_gui.certificatepath));
			...
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				...
				JCert_gui.messagepanel.display(" opening " + filepath);
					...
					if (successful) {
						if (newsource.message != null) {
							JOptionPane.showMessageDialog(
									JCert_gui.browserpanel, "Loading from "
											+ filename + "\n"
											+ newsource.message,
									"Loading Document ",
						...
						JOptionPane.showMessageDialog(JCert_gui.browserpanel,
		...
		if (command.equals("tempload")) {
			JCert_gui.selectCard("Template Loader");
		}
		if (command.equals("user")) {
			JCert_gui.selectCard("User");
		}
	...
	private void displaycert(documentTemplate acert, Document adoc)
	{
		...
		name.setFont(JCert_gui.headingfont);
	...
	{
		for (documentTemplate cert : JCert_gui.templates) {
	...
	public Document loadDocument(String filename)
	{
		try {
			...
			JCert_gui.messagepanel.error(" failed opening " + filename + " "
					+ e);
		}
		return null;
	}
	...
	public void updateDoc(Document doc, String newsavepath)
	{
		...
		utils.addElement(editiondetails, "editor", JCert_gui.username);
