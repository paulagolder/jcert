...
public class utils
{
	...
	{
		String ftphost = JCert_gui.currentuser.getFtphost();
		String ftpuser = JCert_gui.currentuser.getFtpusername();
		String ftpdirectory = JCert_gui.currentuser.getFtpdirectory();
		String ftppassword = JCert_gui.currentuser.getFtppassword();
		
		JCert_gui.templates.clear();
		JCert_gui.selectCard("Messages");
		...
		if (filename.endsWith(".xml")) {
			// try
			{
				...
				for (documentTemplate acertificate : certlist) {
					JCert_gui.templates.add(acertificate);
				}
			} // catch (Exception e3)
				// {
	...
	{
		if(host == null) return;
		JCert_gui.messagepanel.display("Connecting to ftp");
		FTPClient ftp = new FTPClient();
		try {
			ftp.connect(host);
			ftp.login(user, password);
			JCert_gui.messagepanel.display("Connected to " + host + " " + user
					+ " " + directory);
			JCert_gui.messagepanel.display(ftp.getReplyString());
			if (ftp == null)
				return;
			try {
				Thread.sleep(1000);
			...
			for (int i = 0; i < files.length; i++) {
				...
				if (sname.endsWith(".xml")) {
					JCert_gui.messagepanel.display("loading " + sname
							+ " from " + JCert_gui.temppath + "temp_" + sname);
					String ffile = sname;
					FileOutputStream out = new FileOutputStream(
							JCert_gui.temppath + "temp_" + sname);
					boolean result = ftp.retrieveFile(ffile, out);
					out.close();
					if (!result) {
						JCert_gui.messagepanel.display("loaded error in "
								+ sname);
					} else
						utils.loadtemplate(new File(JCert_gui.temppath
								+ "temp_" + sname));
				}
			}
			ftp.logout();
			...
			JCert_gui.messagepanel.display("error Connecting to ftp" + e);
		} catch (IOException e) {
			JCert_gui.messagepanel.display("error2 Connecting to ftp" + e);
		}
	...
	{
		JCert_gui.messagepanel.display("Connecting to dev..");
		String devpath = JCert_gui.certificatepath + "Dev";
